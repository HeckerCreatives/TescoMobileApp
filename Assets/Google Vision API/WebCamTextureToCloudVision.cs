...
public class WebCamTextureToCloudVision : MonoBehaviour {
	...
	public ScanExam scanExam;

    public void InitializeCamera(Action action = null)
    {
	...
	public void RemoveCamera(Action action = null)
    {
		webcamTexture.Stop();
		webcamTexture = null;
		Renderer r = GetComponent<Renderer>();
	...
	public IEnumerator Capture(Action success = null, Action failed = null) {
		while (true) {
			if (this.apiKey == null)
				yield return null;
			...
			if (jsonData != string.Empty) {
				...
				using(WWW www = new WWW(url, postData, headers)) {
					...
					if (string.IsNullOrEmpty(www.error)) {
						try
						{
							TextResponse value = JsonConvert.DeserializeObject<TextResponse>(www.text);
							scanExam.Answers = value.responses[0].fullTextAnnotation.text.ToString().Split('\n').ToList();
							success?.Invoke();
						}
						catch(Exception ex)
                        {
